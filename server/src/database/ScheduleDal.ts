...
export default class ScheduleRepository {
  ...
  static async getOrCreateWeeklySchedule(accountID: Types.ObjectId, date: number) {
    ...
    try {
      const baseSchedule = await ScheduleRepository.getBaseSchedule(accountID as Types.ObjectId);
      const { appointmentLength, week } = baseSchedule;

      const { startingDay, weekLater } = ScheduleRepository.getDateRange(date);

      const uniqueWorkingDates = await ScheduleRepository.getUniqueWorkingDates(startingDay, weekLater, week);
  ...
  static getUniqueWorkingDates(startingDay: Date, weekLater: Date, week: { day: string, startingTime: string, endingTime: string }[]) {
    ...
    for (const workingDay of week) {
        ...
        // Calculate the first matching day from the startingDay
        ...
        // Will get date and transform it to 'Asia/Jerusalem' + Format date according to ISO
        const localISODateString = (date: Date) => {
              const formatter = new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Jerusalem', year: 'numeric', month: '2-digit', day: '2-digit' });
        ...
        for (let date = firstMatchDate; date <= weekLater; date.setDate(date.getDate() + 7)) {
          workingDates.push(localISODateString(date));
        }
    }
    // Convert the dates back to Date and sort
    const uniqueWorkingDates = Array.from(workingDates).map(dateStr => new Date(dateStr));
